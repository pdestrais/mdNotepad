...
export class TagsPage implements OnInit {
	...
	getTags() {
		this.dataService.getDocsOfType('tag').then((data) => {
			//			this.zone.run(() => {
			console.log('[tags - getTags]tags loaded : ' + JSON.stringify(data));
			this.tagList = data;
			this.filteredTags = data.map((value) => value);
			//			});
		});
	}
					...
					result.map((note) => {
						...
						if (i != -1) {
							...
							this.dataService
								...
								.then(() =>
									console.log(
										'[TagPage - renameTag] note saved with new tag : ' + JSON.stringify(note)
									)
								);
						}
					})
	...
	deleteTag() {
		...
		this.dataService
			.deleteDoc(this.selectedTag)
			.then(() => {
				console.log('[tags - deleteTags]tag deleted');
				this.getTags();
				this.dataService.getDocsOfType('note').then((result) => {
					result.map((note) => {
						if (note.tags.includes(this.selectedTag.name)) {
							...
							this.dataService
								...
								.then(() =>
									console.log(
										'[TagPage - deleteTag] note saved without tag : ' + JSON.stringify(note)
									)
								);
						}
					});
					this.showMessage(
						'The tag is deteled (including in the notes where it was used)',
						'middle',
						'light'
					);
					this.tagView = true;
				...
				buttons: [
					...
					},
					{
						...
						handler: (data) => {
							...
							this.dataService.saveDoc(tag, 'tag').then(() => this.getTags());
						}
					}
				]
			})
