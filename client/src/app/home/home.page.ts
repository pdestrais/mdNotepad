...
export class HomePage implements OnInit {
  ...
  async alertNoRemoteDB() {
    const alert = await this.alertCtrl.create({
      ...
      message: 'No Cloudant remote DB is defined to store your notes. Only the local storage will be used and no synchronization between devices will be possible.',
      buttons: [
        {
          text: 'Ok',
          handler: () => {
  ...
  ngOnInit(){
    ...
    this.getAllNotes();
    this.getAllTags();
    // but sometime we have to load the notes data after the synchronizatioin with a remote db is finished
    this.dataService.dataServiceSubject.subscribe(event => {
      if (event.message == "ReplicationCompleted" || event.message == "document deleted" || event.message == "document saved") {
        this.getAllNotes();
        this.getAllTags();
  ...
  fullTextSearch(searchTerm) {
    this.filteredNotes = this.notes.filter(value => value.content.toLowerCase().includes(searchTerm.toLowerCase()));
  }
